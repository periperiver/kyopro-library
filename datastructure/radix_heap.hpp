...
private:
  int sz;
  T last;
  std::vector<std::pair<T,int>>h[1+std::numeric_limits<T>::digits];
  std::vector<std::pair<int,int>>p;
public:
  radix_heap(int n):p(n,std::make_pair(0,-1)),sz(0),last(0){}
  ...
  int pop(){
    if(h[0].empty()){
      ...
      for(int j=0;j<(int)h[b].size();j++){
        ...
        p[i]=std::make_pair(nb,(int)h[nb].size());
        h[nb].emplace_back(std::move(h[b][j]));
      }
      h[b].clear();
    }
    --sz;
  ...
  void decrease_key(int i,T v){
    if(p[i].second<0){
      int b=msb(v^last)+1;
      ++sz;
      p[i]=std::make_pair(b,(int)h[b].size());
      ...
      if(nxtb<preb){
        ...
        std::swap(h[preb][j],h[preb].back());
        p[i2].second=j;
        p[i]=std::make_pair(nxtb,(int)h[nxtb].size());
